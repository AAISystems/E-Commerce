            ...
            @if ($orders->isNotEmpty())
                ...
                @foreach ($orders as $order)
                    <div class="row justify-content-center align-items-top g-2 p-4 shadow-sm">
                                    ...
                                    @foreach ($order->products as $index => $product)
                                        <div class="carousel-item{{ $index === 0 ? ' active' : '' }}">
                                            <img src="{{ asset($product->images->first()->route) }}"
                                                class="img p-4 img-fluid" alt="{{ $product->name }}">
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="text-end"> <a href="{{ route('invoices.create', $order->id) }}">Generar factura</a>
                        </div>
                    </div>
                @endforeach
            @else
                <h2 class="fw-light text-center">Todavía no hay pedidos</h2>
                <div class="d-flex justify-content-center">
                    <img src="{{ asset('img/sad-svgrepo-com.svg') }}" class="img-fluid " alt="cara triste" />


                </div>
                <div class="d-flex justify-content-center">
                    <a href="{{ route('home') }}" class="btn btn-warning p-2 fw-light shadow">Empezar a comprar</a>
